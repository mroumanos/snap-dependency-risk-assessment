...
async def fetch_foodbanks_for_state(session: aiohttp.ClientSession, state: str) -> List[Dict]:
    """
    Fetch foodbank information for a given state from Feeding America
    """
    foodbanks = []
...
async def fetch_all_foodbanks() -> List[Dict]:
    """Fetch foodbanks for all states asynchronously"""
    results = []
...
def process_all_foodbanks(all_foodbanks: List[Dict]) -> List[Dict]:
    ...
    # Expand nested 'MailAddress' dictionaries into separate columns
...
async def run():
    """Run the full ETL process for Feeding America foodbanks"""
